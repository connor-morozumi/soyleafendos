...
```{r}
path = here("CO2_Project","V4_515F_New_V4_806R_New") # CHANGE ME to the directory containing the fastq files after unzipping.
...
```{r}
plotQualityProfile(fnFs[13:14])

plotQualityProfile(fnRs[12])
```

Based on these profiles, we will truncate the reverse reads at position 240 where the quality distribution crashes.
...
We’ll use some standard filtering parameters: maxN=0 (DADA2 requires no Ns), truncQ=2, rm.phix=TRUE and maxEE=2. The maxEE parameter sets the maximum number of “expected errors” allowed in a read, which is a better filter than simply averaging quality scores.

Specific arguments here and reasons: 

- we didn't remove primers before the run so we also add the ` trimLeft=(FWD_PRIMER_LEN, REV_PRIMER_LEN)` argument

- we are having polyG and polyC issues with some of the sequences. After chatting with Ben https://github.com/benjjneb/dada2/issues/1646 and hunting down advice on BioStars:
https://sequencing.qcfail.com/articles/illumina-2-colour-chemistry-can-overcall-high-confidence-g-bases/

https://www.biostars.org/p/294612/

1. We can try adding `rm.lowcomplex` to filterAndTrim. This appears to remove reads where low complexity is detected.
2. Alternatively, we can work to remove these before `dada2` using something like fastp. 
...
for(i in seq_along(fastpFs)){
  print(i)
  message('Processing file ', fastpFs[i], ' of ', length(fastpFs))
  message('And R2 file ', fastpRs[i], ' of ', length(fastpRs))
  
  message('Creating file', fastpF.trim[i])
  message('And file', fastpR.trim[i])
...
# while it's doing all files it isn't saving all files
# and many others are coming back at 0kb / totally empty files... hmmmm
#try smaller runs of files

#ten seems to work, at least these files are now saved 
for(i in 1:10){
  print(i)
  message('Processing file ', fastpFs[i], ' of ', length(fastpFs))
  message('And R2 file ', fastpRs[i], ' of ', length(fastpRs))
...
#lord help us I have no idea why this works, next check and make sure the seqs coming out make sense
for(i in 91:118){
  print(i)
  message('Processing file ', fastpFs[i], ' of ', length(fastpFs))
  message('And R2 file ', fastpRs[i], ' of ', length(fastpRs))
...
Now run filterAndTrim in dada2
```{r}
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(240,240),
              ...
              rm.lowcomplex = 5 # remove low complex
              ) 
...
Alternative error model not currently working (2.22.23) for bacteria dataset
```{r}
...
Sequencing center gave us this target for total fragment length (Expected total fragment length(nt)
 with additional index and adaptors): 395

and said the Expected length(nt) of amplified region
 including locus-specific primer sequences would be: 292
 
 Thus I thought I was expecting 292-19= 273
...
output= here("CO2_Project","outputs")
...
write(asv_fasta, paste0(output, "/","ASVs.16s.",Sys.Date(),".fa"))
...
write.table(asv_tab, paste0(output, "/", "ASVs_counts.16s.",Sys.Date(), ".tsv"), sep="\t", quote=F, col.names=NA)

```
Write taxa csv
```{r}
write.csv(taxa, paste0(output, "/", "taxa.CO2.16s.",Sys.Date(), ".csv"))
```

```{r}
save.image(paste0(path, "/bacteria.RData")) #saves an .RData to the base folder (in this case to the V4_515F... folder). Note these can get really big so might not be worth it 
```

# Work Log

2.22.23: Ran this completely to end with fastP addition. Pulled outputs from AWS onto local to do analyses. Session notes- errmod1 isn't working on this, just used the default dada2 procedure. Created fresh .RData that can be loaded in with this pipeline and outputs from the script as currently saved.
