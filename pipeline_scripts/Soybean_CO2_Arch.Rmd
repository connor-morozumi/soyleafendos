...
```{r}
fnFs.cut <- file.path(path.fastp, basename(fnFs))
fnRs.cut <- file.path(path.fastp, basename(fnRs))
```
...
plotQualityProfile(fnRs.cut[42:46])
```
...
```{r}
out.stand <- filterAndTrim(fnFs.cut, filtFs, fnRs.cut, filtRs, truncLen=c(0,240),
...
the dada2 tutorial has this code:
```{r}
...
cnm notes: that website only had a _r138 version so I downloaded that as a substitute
```{r}
